...
\subsubsection{Ajout des éléments d'évaluation}

Le dernier ajout majeur qui soit visible est l'ajout des éléments d'évaluation. Cela vise à faire de ce fichier un outil complet de travail pour les professeurs, qui peuvent ainsi non seulement accéder à la liste des sujets, mais aussi se faire une idée plus claire des attendus en consultant les éléments d'évaluation sur les épreuves quand ils sont disponibles. Un système de lien et de rétrolien permet de naviguer facilement de l'un à l'autre. On a indiqué dans l'encart au-dessus de chaque sujet si des éléments d'évaluation sont ou non disponibles, et on a créé un lien vers ceux-là quand ils sont bien disponibles. Pour la mise en page des éléments d'évaluation, on a fait en sorte de pouvoir retourner rapidement au sujet par le même système de lien. On a préféré rester sobre dans l'indication de la présence d'éléments d'évaluation, par gain de place d'abord, mais aussi pour que les sujets puissent, malgré tout, être imprimés et donné tel quel si des professeurs en ressentaient le besoin. On trouvera à la fin, avec les autres index, un \hyperlink{indexc}{index des questions avec des éléments d'évaluation}. On trouvera également à la fin, dans la \hyperlink{TdM}{Table des matières}, la liste des sujets disposant d'éléments d'évaluation classés par centres d'examen et session du baccalauréat, ce qui permet d'avoir une vue synoptique.

Pour que ces éléments d'évaluation puissent également être utiles dans la préparation des cours, et non seulement comme le moyen de pouvoir évaluer les attendus sur un sujet donné, on a créé un nouvel \hyperlink{indexr}{index des références culturelles, œuvres et auteurs cités dans les éléments d'évaluation}\footnote{Index dont, à dire vrai, on n'est vraiment peu certain de l'utilité, même si on peut au moins y voir une forme d'intérêt statistique ou historique sur les références jugées pertinentes sur un sujet donné, et qui permet d'identifier des récurrences dans les auteurs utiles.}. On verra, à l'usage, si cela revêt une forme d'utilité et si cela doit être conservé.

\subsubsection{Création d'une version de ce fichier sans les éléments d'évaluation}

Pour les professeurs qui souhaiteraient avoir un fichier moins long (et moins lourd) qui ne contiennent pas les éléments d'évaluation, vous le trouverez ici~: \sujetshlpsanscorr{}.
...
\subsubsection{Création d'une commande conditionnelle pour la création du csv}

Pour inclure le \texttt{csv} on a besoin d'abord de le créer à partir des fichiers qui composent ce document, puis d'inclure chacun des éléments concernant les sujets. On a utilisé le package \texttt{datatool} pour gérer cette inclusion d'un \texttt{csv} conçu comme base de données, et on a créé une commande booléenne à activer (ou non) en début de document pour produire le csv (\texttt{$\backslash$gencsvtrue} pour créer le \texttt{csv} lors de la première compilation, \texttt{$\backslash$gencsvfalse} pour l'utiliser lors des compilations suivantes et inclure les données du \texttt{csv} dans le fichier. C'est ce qui a demandé le plus de changements du point de vue de nos méthodes de travail ordinaire, mais qui est bénéfique pour la gestion des caractères spéciaux et simplifiera, à l'avenir, l'indexation tant des questions pour chaque sujet que des éléments de correction.
