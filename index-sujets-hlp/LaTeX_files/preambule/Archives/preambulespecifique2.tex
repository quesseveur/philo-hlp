...
% 4.3 Formatage du nom de l'auteur pour l'index
\newcommand{\formatauteurindex}{%
    \ifthenelse{\equal{\prenomauteur}{}}%
    {\nomauteur}% Sans prénom
    {%
        \IfEndWith{\initialeprenom}{de}% Avec particule
...
% 5.1 Indexation par auteur
\newcommand{\indexerauteur}{%
    \ifthenelse{\equal{\prenomauteur}{}}%
    {\index[a]{\textsc{\nomauteur}}}% Indexer sans prénom
    {\index[a]{\textsc{\nomauteur}, \prenomauteur}}% Indexer avec prénom
}

% 5.2 Ajout à la table des matières et indexation par sujet
\newcommand{\indexerettoc}{%
    \ifthenelse{\equal{\type}{}}%
    {%
        % Format pour épreuve obligatoire
        \ifthenelse{\equal{\prenomauteur}{}}
        {% Si l'auteur n'a pas de prénom
        ...
        {\IfEndWith{\initialeprenom}{de}%Et qu'il a un nom à particule
        ...
        %Fin du cas à particules
		%Cas sans particules
        {\phantomsection\addcontentsline{toc}{section}{\initialeprenom{}.~\textsc{\nomauteur}, \textit{\titre} (\lieu~\annee, Jour~\jour{})}
        \index[n]{{\annee~\lieu~(Jour~\jour) -- \nomauteur}@\textbf{\annee,~\lieu} (Jour~\jour) -- \\*\hspace*{1em}\initialeprenom{}.~\nomauteur, \textit{\titre}}
        }%Fin du cas sans particules
%        \index[n]{{\annee~\lieu~(Jour~\jour) -- \nomauteur}@\textbf{\annee,~\lieu} (Jour~\jour) -- \\*\hspace*{1em}\formatauteurindex{}, \textit{\titre}}
    }%Fin du cas où l'auteur a un prénom dans une épreuve obligatoire
    }%Fin du cas de l'épreuve obligatoire
    {% Format pour épreuve avec type spécifié
        \ifthenelse{\equal{\prenomauteur}{}}{
        %Si l'auteur n'a pas de prénom
        ...
        }%Fin du cas où l'auteur n'a pas de prénom
        {%Si l'auteur a un prénom
       \IfEndWith{\initialeprenom}{de}%Et qu'il a un nom à particule
       {\phantomsection\addcontentsline{toc}{section}{\initialeprenom{}~\textsc{\nomauteur}, \textit{\titre} (\lieu~\annee~(\type), Jour~\jour{})}
