...
%\newcommand{\sujetcorrige}[8]{
%	\ifthenelse{\equal{#3}{}}{%
%	\fancyhead[C]{\footnotesize Éléments de correction du #2 en #1 (#8)~: \textsc{#5}, \textit{#6}}
%	}
%	{
%	\fancyhead[C]{\footnotesize Éléments de correction du #3 du #2 en #1 (#8)~: \textsc{#5}, \textit{#6}}}
%	\ifthenelse{\equal{#3}{}}{\subsection{#1 (#8), #2}}
%	{\subsection{#1~(#8), #2, #3}}
%\begin{center}
%\fbox{
%\makebox[\linewidth-3.25cm][c]{
...
%	\ifthenelse{\equal{#4}{}}
%	{%
%	\hyperlink{#7}{\textit{Voir le sujet} : \textsc{#5}, \textit{#6}}\hypertarget{corr#7}{}%
%	}
%	{%
%	{\hyperlink{#7}{\textit{Voir le sujet} : #4 \textsc{#5}, \textit{#6}}\hypertarget{corr#7}{}%
%	}}
%	\end{center}
%	\end{minipage}
%	}}
...
\newcommand{\preambulecorr}[1]{\vspace*{-12pt}
\begin{center}
\fbox{\makebox[\linewidth-1.5cm][c]{
\begin{minipage}{\linewidth-2.0cm}
\begin{small}
...
\newcommand{\noteorthographemoyenne}{~\\\begin{scriptsize}Selon la note de service du \href{https://www.education.gouv.fr/bo/2025/Hebdo32/MENE2523939N}{25-8-2025} relative au déroulement des corrections aux examens du
second degré à compter de la session 2026 :\\« Chaque correcteur prend en compte dans l’attribution de la note la qualité rédactionnelle des
candidats : l’orthographe, syntaxe, grammaire, clarté de la langue et lisibilité du propos. Ainsi,
toute copie dont la lecture serait jugée incompréhensible doit se voir attribuer une note inférieure
à la moyenne. […] La situation particulière des candidats bénéficiant d’un aménagement ou
adaptation doit naturellement être prise en compte ».\end{scriptsize}}

\newcommand{\preambulecorrint}{%
\ifnum\c@chapter>2025{{\small On prendra en compte l’intelligibilité, l’expression et la maîtrise de la langue, sans toutefois
pénaliser deux fois mais en considérant ce critère à l’échelle de l’ensemble de la copie.}}\fi

\preambulecorr{L’exercice n’impose ni un nombre de «~parties~», ni un développement obéissant
à une forme prédéfinie ou à une composition canonique~: il ne s’agit pas d’une
explication de texte exhaustive, mais d’une lecture centrée sur certains éléments parmi
les plus significatifs. L’interprétation, guidée par la question, requiert bien évidemment
une attention à la lettre ainsi qu’à la langue du texte, à son écriture (son «~style~»), et
tout particulièrement au questionnement qu’il développe et instruit.

Les propositions qu’on trouvera ci-dessous ne constituent en aucun cas une
correction exhaustive. Elles sont susceptibles d’être enrichies et ajustées au sein des
commissions académiques.

On utilisera tout l’éventail des notes~: on n’hésitera pas à attribuer aux très
bonnes copies des notes allant jusqu’à 10~; la qualité est à évaluer à l’aune de la
compréhension que l’on peut attendre d’un candidat issu de la classe terminale. Les
notes inférieures à 3/10 correspondent à des copies véritablement indigentes.

L’appréciation est précise, nuancée et ne se limite pas à pointer les faiblesses
du devoir~; on se pose prioritairement la question suivante~: «~quelles sont les qualités
de la copie~?~»

L’évaluation des travaux tient compte de la qualité de l’expression (correction
orthographique et syntaxique~; précision, justesse, finesse, voire élégance de la
rédaction).
\ifnum\c@chapter>2025{\noteorthographemoyenne}\fi}%
}

\newcommand{\preambulecorrref}{
\preambulecorr{L’essai n’impose ni un nombre de «~parties~», ni un développement obéissant à
une forme prédéfinie ou à une logique de composition canonique. En revanche, il
suppose une implication personnelle dans la réflexion favorisant l’exploration et
l’exploitation de connaissances que les candidats ont pu s’approprier.
...
On utilisera tout l’éventail des notes~: on n’hésitera pas à attribuer aux très
bonnes copies des notes allant jusqu’à 10~; la qualité est à évaluer par rapport aux
connaissances et capacités que l’on peut attendre d’un candidat issu de la classe
terminale. Les notes inférieures à 3/10 correspondent à des copies véritablement
indigentes.

L’appréciation est précise, nuancée, elle ne se limite pas à pointer les faiblesses
du devoir. On se pose la question suivante~: «~quelles sont les qualités de la copie~?~»
...
\vspace*{-6pt}\begin{center}{\textbf{\underline{Sujet~:}} {#2}}\end{center}\vspace*{-6pt}
\preambulecorrint}

\newcommand{\corrref}[2]{
\subsubsection{Essai #1}
\vspace*{-6pt}\begin{center}{\textbf{\underline{Sujet~:}} {#2}}\end{center}\vspace*{-6pt}
\preambulecorrref}

% #1 = Prénom ; #2 = Nom
\newcommand{\corrauteur}[2]{#2%
  \ifthenelse{\equal{#2}{}}{}{%
    \ifthenelse{\equal{#1}{}}%
      {\index[r]{#2@\textsc{#2}}}%
      {\index[r]{#2@\textsc{#2}~(#1)}}%
...
% #1 = Prénom ; #2 = Nom ; #3 = Titre
\newcommand{\corroeuvre}[3]{\emph{#3}%
  \ifthenelse{\equal{#2}{}}{}{% Auteur vide ? On ne fait rien.
    \ifthenelse{\equal{#3}{}}%
      {\index[r]{#2@\textsc{#2}}}% #3 est vide : pas de "!"
      {\index[r]{#2@\textsc{#2}!#3}}% #3 existe : on met le "!"
  }%
  \xspace%
}

%#1 = Prénom, #2 = Nom, #3 = Titre, #4 = Texte à afficher dans le corrigé
\newcommand{\corrreference}[4]{#4%
  \ifthenelse{\equal{#2}{}}{}{% Auteur vide ? On ne fait rien.
    \ifthenelse{\equal{#1}{}}%
      {% Cas sans particule
        \ifthenelse{\equal{#3}{}}%
          {\index[r]{#2@\textsc{#2}}}% Pas d'œuvre : pas de "!"
          {\index[r]{#2@\textsc{#2}!#3}}% Œuvre existe
      }%
      {% Cas avec particule (ou variante)
        \ifthenelse{\equal{#3}{}}%
          {\index[r]{#2@\textsc{#2}~(#1)}}% Pas d'œuvre
          {\index[r]{#2@\textsc{#2}~(#1)!#3}}% Œuvre existe
      }%
  }%
}
...
\newcommand{\sujetcorrigen}[3]{
	...
	\fancyhead[C]{\footnotesize Éléments d'évaluation du jour~\corrjour{}, \corrlieu~(\corrannee)~: \textsc{\corrnomauteur}, \textit{\corrtitre}}
	\subsection{\corrlieu{}~(\corrannee),~Jour~\corrjour}
	}
	{\fancyhead[C]{\footnotesize Éléments d'évaluation du jour~\corrjour{}, \corrlieu~(\corrtype~\corrannee)~: \textsc{\corrnomauteur}, \textit{\corrtitre}}
	\subsection{\corrlieu{}~(\corrtype~\corrannee),~Jour~\corrjour}}
	}
	{%
	\ifthenelse{\equal{\corrtype}{}}
	{\fancyhead[C]{\footnotesize Éléments d'évaluation du sujet~\corrnosujet~du~jour~\corrjour{}, \corrlieu~(\corrannee)~: \textsc{\corrnomauteur}, \textit{\corrtitre}}
	\subsection{\corrlieu~(\corrannee),~Jour~\corrjour,~Sujet~\corrnosujet}}
	{\fancyhead[C]{\footnotesize Éléments d'évaluation du sujet~\corrnosujet~du~jour~\corrjour{}, \corrlieu~(\corrtype~\corrannee)~: \textsc{\corrnomauteur}, \textit{\corrtitre}}	\subsection{\corrlieu~(\corrtype~\corrannee),~Jour~\corrjour,~Sujet~\corrnosujet}}
...
\begin{minipage}{\linewidth-3.25cm}
	\begin{center}
	\noindent {\Large
	\textit{Éléments d'évaluation}}\par
