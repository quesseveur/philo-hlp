...
Après chaque génération réussie, le script conserve automatiquement **les deux fichiers les plus récents** correspondant exactement au modèle :

```text
ajout-index_YYYYMMDD.html
```

Les versions plus anciennes sont supprimées. Le nettoyage ne touche **ni** `index.tex`, **ni** `carte-mentale.html`, **ni** les sauvegardes de la forme `carte-mentale.sauvegarde-YYYYMMDD-HHMMSS.html`.

Cette conservation de deux versions est intentionnelle : elle laisse toujours le fragment actuel et le fragment précédent pour une restauration manuelle rapide. Si vous souhaitez conserver davantage de fragments, utilisez par exemple :

```bash
python3 generer_index_thematique.py --keep-fragments 4
```

La valeur ne peut pas descendre sous deux : le script refusera une valeur inférieure afin de préserver votre possibilité de retour en arrière.

## Nettoyage approfondi à la demande

Si vous voulez aussi nettoyer les sauvegardes horodatées créées lors des mises à jour de `carte-mentale.html`, lancez explicitement :

```bash
python3 generer_index_thematique.py --deep-clean
```

Cette commande **ne génère pas** de nouveau fragment et ne modifie pas la page. Elle supprime seulement les fichiers correspondant exactement aux deux modèles suivants :

```text
ajout-index_YYYYMMDD.html
carte-mentale.sauvegarde-YYYYMMDD-HHMMSS.html
```

Elle conserve, pour chaque modèle, **une seule version : la plus récente**. Les autres fichiers du dossier, y compris `index.tex`, `carte-mentale.html`, les scripts JavaScript et tout fichier ne correspondant pas exactement à ces noms, ne sont pas concernés.

> Utilisez cette commande seulement lorsque vous estimez ne plus avoir besoin de conserver deux versions de retour. La génération ordinaire conserve volontairement deux fragments ; le nettoyage approfondi est une action distincte et explicite.
